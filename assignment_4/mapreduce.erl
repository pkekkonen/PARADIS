...
mapreduce(Mapper, Mappers, Reducer, Reducers, Input) ->
    ...
    [Pid ! {done, Reducers} || Pid <- ReducerPids],
...
spawn_mapper(Master, Ref, Mapper, Reducer, Reducers, Data) ->
    spawn_link(fun () ->
		       ...
		       case is_process_alive(whereis(erlang:phash(MapKey, Reducers))) of
		       		true ->
		       			%skicka till den piden
		       			erlang:phash(MapKey, Reducers) ! {map, {self(), Ref, groupkeys(lists:sort(Map))}};
		       		false ->
		       			%skapa process, registrera 
		       			Ref = make_ref(),
		       			ReducerPid = spawn_reducer(Master, Ref, Reducer),
		       			register(erlang:phash(MapKey, Reducers), ReducerPid)
		       	end
	       end).
